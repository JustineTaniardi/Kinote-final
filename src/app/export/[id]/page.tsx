...
export default function ExportPage() {
  ...
  useEffect(() => {
    if (!streakId) return;
    fetchExportData();
  }, [streakId, selectedMonth, selectedYear]);
  ...
  const fetchExportData = async () => {
    try {
      ...
      const response = await fetch(
        `/api/streaks/${streakId}/export?month=${selectedMonth}&year=${selectedYear}`,
        {
          headers: {
  ...
  const generateShareToken = async () => {
    try {
      const token = localStorage.getItem("authToken");
      const response = await fetch(`/api/streaks/${streakId}/export`, {
        method: "POST",
        headers: {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => router.back()}
            className="px-4 py-2 bg-[#0f1a31] text-white rounded-lg hover:bg-[#1a2847] transition"
          >
            ← Go Back
          </button>
        </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <button
              onClick={() => router.back()}
              className="inline-flex items-center gap-2 px-4 py-2 text-gray-600 hover:text-gray-900 hover:bg-white rounded-lg transition mb-4"
            >
              ← Back
            </button>
